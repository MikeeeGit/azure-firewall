...
## Gateway API platform dependencies

For the maintained Envoy/CSI profile, review the [explicit platform egress layer](../examples/aks-platform/README.md) before installing the controller. It adds the pinned images' registry/auth/CDN path and the exact PPRD application vault, scoped to node sources. The default AKS/ACR rule is not sufficient for those dependencies.
